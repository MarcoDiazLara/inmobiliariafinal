...
@Component({
  selector: 'app-republicar',
  templateUrl: './republicar.component.html',
  styleUrls: ['./republicar.component.scss']
})
export class RepublicarComponent implements OnInit {
  ...
  CerraDialogo(){
    ...
    // "estatus: " + estatus)   
      ...
      this.httpService.republicar(nombre,Descripcion,prec_min, prec_max,prec_final, usuario,inmueble, estatus).subscribe((data:any)=> {
        if(data==1){
                
                Swal.fire(
                  'Exitosamente!',
                  'Se ha resubido la informacion',
                  'success'
